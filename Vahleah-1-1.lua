...
local UIMainWindow, WindowContent do
	...
	if (SaveData.SkipIntro and math.random(2) == 1) or os.date("%m %d") ~= "04 01" then
		local quotes = {
			"Vahleah Re-create | v" .. UhhhhhhVersion,
			"jumpscare",
		...
		if not aprilfools then
			task.delay(2, function()
				TopBarText.Text = "Vahleah Reanimate | v" .. UhhhhhhVersion
			end)
		end
	end
...
task.spawn(function()
	...
	local AsciiTextarts = {
		...
		},
		{
			...
			"Uhhhhhh Uhhhhhh Uhhhhhh Uhhhhhh LEOTHEREALONE",
			"Uhhhhhh Uhhhhhh Uhhhhhh LEOTHEREALONE Uhhhhhh",
			"Uhhhhhh Uhhhhhh LEOTHEREALONE Uhhhhhh Uhhhhhh",
			"Uhhhhhh LEOTHEREALONE Uhhhhhh Uhhhhhh Uhhhhhh",
			"LEOTHEREALONE Uhhhhhh Uhhhhhh Uhhhhhh Uhhhhhh",
		},
	}
...
function HatReanimator.Config(parent)
	...
	UI.CreateButton(parent, "Rebuild Hat Map", 20).Activated:Connect(function()
		HatReanimator.RebuildRequired = true
		local msg = "-gh 139904067056008 108186273151388 72292903231768 95290698984301 84451219120140 17553329441 117311153426168 116940095199813 5316479641 4504231783 107682077356653 109795089349040"
		-- TextChatService (new chat)
		pcall(function()
			local channel = TextChatService:FindFirstChildOfClass("ChatInputBarConfiguration")
			if channel and channel.TargetTextChannel then
				channel.TargetTextChannel:SendAsync(msg, Util.RandomString(64))
			end
		end)
...
do
	...
	ReanimateStartButton.Activated:Connect(function()
		...
		if Reanimate.Current then
			-- Immediately chat -re with no delay when deanimating
			pcall(function()
				local channel = TextChatService:FindFirstChildOfClass("ChatInputBarConfiguration")
				if channel and channel.TargetTextChannel then
					channel.TargetTextChannel:SendAsync("-re", Util.RandomString(64))
				end
			end)
			...
			-- Chat -pd immediately, -net after 1.5 second delay
pcall(function()
	local channel = TextChatService:FindFirstChildOfClass("ChatInputBarConfiguration")
	if channel and channel.TargetTextChannel then
		channel.TargetTextChannel:SendAsync("-pd", Util.RandomString(64))
	end
end)
task.delay(1.5, function()
	pcall(function()
		local channel = TextChatService:FindFirstChildOfClass("ChatInputBarConfiguration")
		if channel and channel.TargetTextChannel then
			channel.TargetTextChannel:SendAsync("-net", Util.RandomString(64))
		end
	end)
end)
