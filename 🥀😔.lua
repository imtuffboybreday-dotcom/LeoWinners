...
if not game:IsLoaded() then
	...
	notLoaded.Text = "Vahleah is waiting for the game to load"
	game.Loaded:Wait()
	for i=1, 60 do
...
if SaveData.SkipIntro then
	...
	UISound.Music.TimePosition = 0
	local scrolltexts = {
		"LEONEVERALONE presents a script                                 ",
...
local CracktroFrameText = "Vahleah Reanimate 1" .. UhhhhhhVersion
local UIMainWindow, WindowContent do
	...
	TopBarText.Text = "Vahleah Reanimate | 1" .. UhhhhhhVersion
	...
	if (SaveData.SkipIntro and math.random(2) == 1) or os.date("%m %d") ~= "04 01" then
		local quotes = {
			"Vahleah Re-create | 1" .. UhhhhhhVersion,
			"jumpscare",
			"you'll never see it coming",
			Player.Name .. "'s Vahleah Session",
			...
			"Delta1 Reanimate | 1" .. UhhhhhhVersion,
		}
...
do -- homepage
	...
	local text0 = Util.MakeText("Discord: https://discord.gg/TMmyPgRs7")
	text0.AnchorPoint = Vector2.new(0.5, 1)
	text0.Position = UDim2.new(0.5, 0, 1, -17)
	text0.ZIndex = 3
	text0.Parent = CracktroFrame
	local text1 = Util.MakeText("Made by LEONEVERALONE :" .. (math.random() < 0.333 and "3" or (math.random() < 0.5 and "D" or "P")))
	...
	local quotes = {
		...
		"You are going to love Fahhh, I just know it.",
...
task.spawn(function()
	...
	local AsciiTextarts = {
		...
		{ -- omg hi steveee-
			...
			":;:;:; qm#WWNWQQWQWWmp ;:;:;:.                  .;:;",
			"!!?!!?.WX##: W#W :###W.?!!?:.  hi im Leo!      .:!",
			"lilili:ZSXS: XSX :X##Z:ilil!. welcome to Vahleah .!i",
			"EEEEEE:YLkk2S2SXSXSXZP:EEEEEi:                  :iEE",
			"%%%%%%%;vnvkkkk2S2vnv;%%%%%%%%%oooooooooooooooo%%%%%",
		},
		{
			"  Get Vahleah today! Feel the difference!       ",
			"           (-_-)         EPIC!    (^~^)   .vv=E ",
			"       ()-(:::::)-()         .()-(:::::)-()'    ",
			"LAME!  || |.....| ||      3=^^'  /...../        ",
			"       YP |     | YP            (     )   COOL! ",
			"      Without Vahlah   vs.   With Vahleah      ",
		},
		{
			...
			"68 68 68 68 Vahleah  68 68 68 68 55 68 68",
		...
		},
		{
			...
			"the dev portal cuz Fahhhh.                        ",
			"                                                   ",
			"\"it's not fast it's shit\"                          ",
			"Believe me friend, its fuckingly fast, you don't ev",
		},
		{
			"Vahleah  https://discord.gg/TMmyPgRs7  Vahleahh",
			"Vahleahh Vahleahh Vahleahh Vahleahh LEOTHEREALONE",
			"Vahleahh Vahleahh Vahleahh LEOTHEREALONE Vahelahh",
			"Vahleahh Vahleahh LEOTHEREALONE Vahleahh Vahleahh",
			"Vahleahh LEOTHEREALONE Vahleahh Vahleahh Vahleahh",
			"LEOTHEREALONE Vahleahh Vahleahh Vahelahh Vahleahh",
		},
	}
...
UI.CreateText(MainPage, `Reanimate V{UhhhhhhVersion}, By LEO :D`, 15, Enum.TextXAlignment.Right)
...
UI.CreateSwitch(MainPage, math.random(8) == 1 and "Mute TIKITIKIMATETETAKAI" or "Mute UI Music", SaveData.MuteUIMusic).Changed:Connect(function(value)
...
local function CreateHumanoidCharacter()
	...
	char.Name = "(C) Vahleah 1" .. UhhhhhhVersion
...
HatReanimator.HatMapSummary = "(no hat map yet, please * Reanimate)"
...
function HatReanimator.Config(parent)
	UI.CreateText(parent, "permadeath 100%Work", 10, Enum.TextXAlignment.Center)
	...
	UI.CreateSwitch(parent, "Hat Fingering", HatReanimator.HatCollide).Changed:Connect(function(val)
	...
	UI.CreateDropdown(parent, "Torso Offset", {
		...
		"9 - (Most Stable) Anti HatdropByVahCommunity",
	...
	UI.CreateText(parent, "^^^ FAH ^^^", 10, Enum.TextXAlignment.Center)
	...
	UI.CreateButton(parent, "GiveHatsRigs", 20).Activated:Connect(function()
		HatReanimator.RebuildRequired = true
		local msg = "-gh 139904067056008 108186273151388 72292903231768 95290698984301 84451219120140 17553329441 117311153426168 116940095199813 5316479641 4504231783 107682077356653 109795089349040 150381051 150381051"
		pcall(function()
			game.ReplicatedStorage["01_server"]:FireServer("cmd", msg)
		end)
	end)
	UI.CreateText(parent, "^^^ Give Hats For Free Rigs ^^^\nthis button is for you", 10, Enum.TextXAlignment.Center)
	UI.CreateButton(parent, "Clearhats", 20).Activated:Connect(function()
		local msg = "-ch"
		pcall(function()
			game.ReplicatedStorage["01_server"]:FireServer("cmd", msg)
		end)
...
function HatReanimator.Start()
	...
	local function CreatePlaceholder(hat)
		...
		if h and h:IsA("BasePart") then
			...
			p.CanQuery = false
			p.Transparency = 0.75
			p.Name = "(C) Vahleah 1" .. UhhhhhhVersion .. " :: HAT PLACEHOLDER"
...
do
	...
	ReanimateStartButton.Activated:Connect(function()
		...
		if Reanimate.Current then
			-- Immediately fire -re with no delay when deanimating
			pcall(function()
			...
			-- Fire -pd immediately, -net after 1.3 second delay
			pcall(function()
				game.ReplicatedStorage["01_server"]:FireServer("cmd", "-pd")
			end)
...
UI.CreateButton(MainPage, "Move &gt;", 20).Activated:Connect(function()
	MovesetsPage.Interactable = false
...
local function AddMoveset(m)
	if type(m) == "table" then
		...
		Util.LinkDestroyI2C(item, item.Activated:Connect(function()
			...
			page.Visible = true
			UI.CreateButton(page, " &lt; Combeck", 20).Activated:Connect(function()
				page.Interactable = false
				MovesetsPage.Interactable = false
...
UI.CreateText(CreditsPage, "Modified by LEO :D", 15, Enum.TextXAlignment.Right)
...
--[[UI.CreateText(CreditsPage, "<font color=\"#4444FF\"><b>Empyrean Reanimate (click for Discord)</b></font>", 12, Enum.TextXAlignment.Center).InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		pcall(setclipboard, "https://discord.gg/TMmyPgRs7")
		pcall(request, {
...
do
	local quotes = {
		...
		"LEO, THE DANCING IMMORTALITY LORD!",
		...
		"\"It's not even another Reanimate; it just uses Emper Reanimate. So you can change the name to 'Vahleah Hub', that's it.\"",
...
UI.CreateSeparator(CreditsPage)
UI.CreateText(CreditsPage, "<b>(C) 2026 LEOTHEREALONE</b>", 14, Enum.TextXAlignment.Center)
	...
	for i,x in filesofbuiltins_d do
		...
		if exist then
			...
			if content then
				...
				InitLogsText.Text ..= "\n[ERROR] Failed to load MAP " .. x .. ": DOWNLOAD FAILED U BITCH."
	...
	for i,x in filesofbuiltins_m do
		...
		if exist then
			...
			if content then
				...
				InitLogsText.Text ..= "\n[ERROR] Failed to load VANILLA " .. x .. ": FAH U THINK ITS WORK?."
				SaveData.ContentHash[x] = nil
			end
		end
		task.wait()
...
clearcontenthash.Activated:Connect(function()
	...
	elseif clearcontenthashclicks == 2 then
		clearcontenthashtext.Text = "dont do it ur motherfuck"
		task.wait(1)
...
local d = function()
	...
	if megadetected then
		...
		task.wait(1.5)
		Util.UINotify("you definitely got kicked>:3")
		task.wait(8)
	end
